...
public class ExistDBConnector {
    ...
    private static final Logger log = LogManager.getLogger(ExistDBConnector.class);
    ...
    public ArrayList<String> insertTokenInPericopeById(String URI, String pericopeId, String node) {
        log.debug("URI=(" + URI + "), pericopeId=(" + pericopeId + "), node=(" + node + ")");
    ...
    /**
     * Recupero tutti gli identificativi dei token appartenenti ad una
     * determinata pericope
     *
     * @param URI
    ...
    /**
     * Ricerca in linkAnalysis.xml l'elemento che ha il riferimento al token di
     * tokenRef uguale a tokenId e restituisce il valore del ref alla analisi
     * morfologica
     *
     * @param URI
     * @param tokenId
    ...
    public String searchLinkIdByPericopeIdAndLang(String URI, String pericopeId, String lang) {
        log.debug("URI=(" + URI + "), pericopeId=(" + pericopeId + "), lang=(" + lang + ")");
    ...
    /**
     * Recupera le informazioni morfologiche (morphoDB.xml) di una entrata con
     * id <i>analysisId</i>
     *
     * @param URI
     * @param analysisId Identificativo della analisi da recuperare
     * @return HashMap contenente le informazioni morfologiche. Le keys sono le
     * etichette (forma, lemma, pos) mentre i values sono i valori ad esse
     * associate
     */
    public HashMap<String, String> searchArabicAnalysisById(String URI, String analysisId) {
        log.debug("URI=(" + URI + "), analysisId=(" + analysisId + ")");
    ...
    public ArrayList<String> searchAnalysisIdByParameter(String URI, List<String> itemName, List<String> itemValue, int lang) {
        log.debug("URI=(" + URI + "), itemName=(" + itemName + "), itemValue=(" + itemValue + "), lang=(" + lang + ")");
    ...
    public ArrayList<String> searchTokenIdByForma(String URI, String forma, int lang) {
        log.debug("URI=(" + URI + "), forma=(" + forma + "), lang=(" + lang + ")");
    ...
    private ArrayList<String> searchAnalysisIdByParameter(String URI, String forma, String pos, String lemma, String radice) {
        log.debug("URI=(" + URI + "), forma=(" + forma + "), pos=(" + pos + "), lemma=(" + lemma + "), radice=(" + radice + ")");
    ...
    public ArrayList<String> searchTokenIdByAnalysisId(String URI, String analysisId) {
        log.debug("URI=(" + URI + "), forma=(" + analysisId + ")");
     ...
     *
     * @param URI It Identifies the database (e.g.
     * xmldb:exist://localhost:8085/exist/xmlrpc/db/ga/new_GA2/doc/bada)
    ...
    public ArrayList<String> xqRunWithMultipleResults(String URI, String xquery) {
        log.debug("URI=(" + URI + "), xquery=(" + xquery + ")");
            ...
            if (i.hasMoreResources()) {
                ...
                while (i.hasMoreResources()) {
                    try {
                        ...
                        log.debug(">>> " + res.getContentAsDOM());
                        NodeList nodeList = node.getChildNodes();
                        for (int index = 0; index < nodeList.getLength(); index++) {
                            log.debug(node.getNodeName() + " " + nodeList.item(index));
                        }
                        //ret.add((String)res.getContent());
                    } finally {
                        //dont forget to cleanup resources
    ...
    /**
     * Controlla l'esistenza del path (al file) individuato da URI nella
     * database dbName (es. "/new_GA/doc/bada", "tokens.xml")
     *
     * @param dbName
     * @param URI
     * @return
